...
from engine_finetune import finetune_fm, evaluate_disease
...
def get_args_parser():
    ...
    parser.add_argument('--adddne', action='store_true')

    parser.add_argument("--dne_path", default=None, type=str)
...
def main(args):
    ...
    subgroups = ["male_nofinding", "male_{}".format(args.disease.lower()), "female_nofinding", "female_{}".format(args.disease.lower())]

    subgroup_datasets = {}
    for subgroup in subgroups:
        dataset = build_dataset_chest_xray(split='test', args=args, subgroup=subgroup)
        subgroup_datasets[subgroup] = dataset
    ...
    if args.finetune and not args.eval:
        if 'vit' in args.model:
            ...
            raise NotImplementedError("Only ViT is supported at this point")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % args.eval_interval == 0 or epoch + 1 == args.epochs):
            ...
            if args.adddne:
                test_stats = evaluate_disease(data_loader_test, model, device, args, dne_layer)
                result_dict = {key: None for key in subgroup_datasets}
                result_dict["overall"] = test_stats['acc1']
                for sub_name, sub_dset in subgroup_datasets.items():
                    loader = torch.utils.data.DataLoader(sub_dset, batch_size=1, shuffle=False)
                    eval_stat = evaluate_disease(loader, model, device, args, dne_layer)
                    result_dict[sub_name] = eval_stat['acc1']
            else:
                test_stats = evaluate_disease(data_loader_test, model, device, args)
                result_dict = {key: None for key in subgroup_datasets}
                result_dict["overall"] = test_stats['acc1']
                for sub_name, sub_dset in subgroup_datasets.items():
                    loader = torch.utils.data.DataLoader(sub_dset, batch_size=1, shuffle=False)
            ...
            eo_p = abs(result_dict['male_pleural'] - result_dict['female_pleural'])
            di = 1 - (result_dict['male_pleural'] / result_dict['female_pleural'])
