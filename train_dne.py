...
from engine_finetune import train_dne
...
def main(args):
    ...
    model_sa.to(device)
    dne_layer = DNELayer((224,224)).to(device)
    ...
    if args.distributed:
        ...
        dne_layer = torch.nn.parallel.DistributedDataParallel(dne_layer, device_ids=[args.gpu])
        dne_layer_without_ddp = dne_layer.module
    ...
    biased_checkpoint = torch.load(args.sa_path, map_location='cpu')
    print("Load pre-trained gender classifier from: %s" % args.sa_path)
    ...
    torch.save(dne_layer_without_ddp.state_dict(), os.path.join(args.output_dir, "dne_layer.pt"))
